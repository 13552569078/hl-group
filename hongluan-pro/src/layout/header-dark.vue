<template show-source>
  <hl-header>
    <div class="logo">
      <img :src="logoEcUrl" alt="hongluan.ui" />
      <h1>综合应急指挥平台</h1>
      <span class="sub-title">{{ version }}</span>
    </div>
    <hl-group class="header-left" gap="var(--xs)">
      <hl-button type="link" class="text-inverse hover:text-light" @click="openVisualizedPage">可视化大屏</hl-button>
      <hl-button
        type="link"
        class="text-inverse hover:text-light"
        @click="openHl"
      >Hongluan.Ui</hl-button>
    </hl-group>
    <div class="header-right">
      <hl-group gap="var(--xs)" class="m-r-xxl">
        <hl-button type="primary" class="text-inverse" no-fill equal @click="openThemes = true">
          <hl-icon>
            <fill-editor-measure />
          </hl-icon>
        </hl-button>
        ...
        >
          <template #reference>
            <hl-button type="primary" class="text-inverse" no-fill equal>
              <template #icon>
                <hl-icon>
                  <fill-mail />
                </hl-icon>
              </template>
              <hl-badge type="danger" position="rt" :value="200" :max="99" round />
            </hl-button>
          </template>
          <hl-group
            dir="vertical"
            align="items-left items-between"
            full
          ...
          <hl-group
            full
            class="p-md border-top cursor-pointer text-primary-hover transition"
            align="items-center"
          >查看全部消息</hl-group>
        </hl-popover>
        <hl-button
          ...
          no-fill
          equal
          @click="$emit('open-drawer', 'right', 'drawer')"
        >
          <hl-icon>
            <fill-layout />
          </hl-icon>
      ...
      <div class="userinfo">
        ...
        <hl-dropdown class="m-l-xs">
          <hl-thumb radius type="primary" size="sm" light class="face">
            <span>松</span>
          </hl-thumb>
          <template #dropdown>
            <hl-dropdown-menu>
...
import { logoEcUrl } from '@/utils/icons'
