...
import installStyle from '@hongluan-ui/test-utils/style-plugin'
import Checkbox from '@hongluan-ui/components/checkbox/src/checkbox.vue'
import CheckboxGroup from '@hongluan-ui/components/checkbox/src/checkbox-group.vue'
...
describe('Form', () => {
  ...
  test('label width', async () => {
    const wrapper = mount({
      setup() {
        ...
        return () => (
          <Form ref="form" model={form} width="80px">
    ...
    expect(findStyle(wrapper, '.form-label').width).toBe('80px')
  })
  ...
  test('auto label width', async () => {
    ...
    const formItems = wrapper.findAll<HTMLElement>('.form-content')
    const marginLeft = Number.parseInt(formItems[0].element.style.marginLeft, 10)
    const marginLeft1 = Number.parseInt(formItems[1].element.style.marginLeft, 10)
    expect(marginLeft).toEqual(marginLeft1)

    labelPosition.value = 'left'
    await nextTick()

    const formItems1 = wrapper.findAll<HTMLElement>('.form-content')
    const marginRight = Number.parseInt(formItems1[0].element.style.marginRight, 10)
    const marginRight1 = Number.parseInt(formItems1[1].element.style.marginRight, 10)
  ...
  test('label position', () => {
      ...
      setup() {
        ...
        return () => (
          <div>
            ...
            <Form model={form} labelPosition="left">
              <FormItem ref="labelLeft">
                <Input v-model={form.name} />
              </FormItem>
              <FormItem>
                <Input v-model={form.address} />
    ...
    expect(wrapper.findComponent({ ref: 'labelTop' }).find('.top')).not.toBeUndefined()
    expect(wrapper.findComponent({ ref: 'labelLeft' }).find('.left')).not.toBeUndefined()
  })
    ...
    const wrapper = mount({
      setup() {
        ...
        return () => (
          <div>
            <div>
              <Form model={form} size="sm" ref="labelSmall">
                <FormItem>
                  <Input v-model={form.name} />
                </FormItem>
              </Form>
  ...
  it('show message', async () => {
    ...
    expect(wrapper.find('.form-content-info').exists()).toBe(false)
  ...
  test('reset field', async () => {
    ...
    expect(wrapper.findAll('.form-content-info')).toHaveLength(0)
    vi.useRealTimers()
  })
  ...
  test('validate return parameters', async () => {
    ...
    function validate() {
      return (vm.$refs.formRef as FormInstance)
        ...
        .catch((fields: any) => ({ valid: false, fields }))
    }
  ...
  describe('FormItem', () => {
    ...
    it('should dynamically register form with items', async () => {
      ...
      await findDeleteDomainButton()[1]!.trigger('click')
      expect(findDomainItems()).toHaveLength(1)
      await findSubmitButton().trigger('click')
      // wait for AsyncValidator to be resolved
      await rAF()
