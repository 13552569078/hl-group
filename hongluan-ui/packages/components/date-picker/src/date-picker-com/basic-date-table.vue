<template>
  <table
    role="grid"
    :aria-label="t('hl.datepicker.dateTablePrompt')"
    cellspacing="0"
    cellpadding="0"
    class="picker-table"
  ...
  >
    <tbody ref="tbodyRef">
      <tr>
        <th v-if="showWeekNumber" scope="col">{{ t('hl.datepicker.week') }}</th>
        <th
          v-for="(week, key) in WEEKS"
          :key="key"
          scope="col"
          :aria-label="t('hl.datepicker.weeksFull.' + week)"
        >
          {{ t('hl.datepicker.weeks.' + week) }}
        </th>
      </tr>
      <tr
...
import { castArray } from '@hongluan-ui/utils'
