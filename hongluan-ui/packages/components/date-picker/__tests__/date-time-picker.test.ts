...
describe('Datetime Picker', () => {
  ...
  it('both picker show correct formated value (extract date-format and time-format from format property', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const dateInput = document.querySelector('.picker-time-header > .group-item:nth-child(1) input')
    const timeInput = document.querySelector('.picker-time-header > .group-item:nth-child(2) input');
  ...
  it('both picker show correct value', async () => {
    ...
    let spinners = document.querySelectorAll('.time-spinner ul li.active') as any
    ...
    await nextTick()
    spinners = document.querySelectorAll('.time-spinner ul li.active') as any
  ...
  it('click now button', async () => {
    ...
    input.trigger('focus')
    await nextTick();
    (document.querySelector(`.picker-footer .${prefix}-button.link`) as HTMLElement).click()
    await nextTick()
  ...
  it('timepicker select && input time && input date', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const input_ = document.querySelectorAll('.picker-time-header > .group-item input')[1];
    (input_ as HTMLElement).focus()
    await nextTick()
    const timePanel = document.querySelector(`.${prefix}-time-picker`)
    expect(timePanel?.querySelector('.time-spinner')?.innerHTML).not.toBeNull()
    const button = document.querySelector(`.${prefix}-time-picker .confirm`) as HTMLElement
    button.click()
    await nextTick()
    expect(vm.value).not.toBe('')
    const timeInput = document.querySelectorAll('.picker-time-header > .group-item input')[1] as HTMLInputElement
    ...
    const dateInput = document.querySelector('.picker-time-header > .group-item:nth-child(1) input') as HTMLInputElement
  ...
  it('now button: can not choose disabled date', async () => {
    ...
    await nextTick()
    // click now button
    const btn = document.querySelector(`.picker-footer .${prefix}-button.link`) as HTMLElement
    btn.click()
    await nextTick()
    const vm = wrapper.vm as any
    expect(vm.value).toBe('')
  ...
  it('confirm button honors picked date', async () => {
    ...
    (document.querySelector('.picker-body-header .next-month') as HTMLElement).click();
    (document.querySelector('.picker-body-header .next-year') as HTMLElement).click();
    // click confirm button
    (document.querySelector(`.picker-footer .${prefix}-button.primary`) as HTMLElement).click()
    const vm = wrapper.vm as any
  ...
  it('selectableRange', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const input1 = document.querySelectorAll('.picker-time-header > .group-item input')[1] as HTMLInputElement
    input1.blur()
    input1.focus()
    await nextTick()
  ...
  it('defaultTime takes effect when the type is datetime', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const someDateTd = document.querySelector('.picker-body-content tr:nth-child(3) td:nth-child(4)')
    const timeInput = document.querySelector('.picker-time-header .group-item:nth-child(2) input');
    ...
    // time spinner highlight is correct
    const spinners = document.querySelectorAll('.time-spinner ul li.active') as any
  ...
  it('defaultTime only takes effect when time is not selected', async () => {
    ...
    const timeInput = document.querySelector(
      '.picker-time-header > .group-item:nth-child(2) input',
    ) as HTMLInputElement
    expect(timeInput.value).toBe('12:00:00')
    ;(timeInput as HTMLElement).focus()
    await nextTick()
    const spinner = document.querySelector(
      '.time-spinner ul li.active',
    ) as HTMLElement
...
describe('Datetimerange', () => {
  ...
  it('select daterange and default Time and input format', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const pickers = document.querySelectorAll('.picker-body-content')
    ...
    const pickerss = document.querySelectorAll('.picker-body-header-wrapper .picker-time-header')
    const left = {
      dateInput: pickerss[0].querySelector('.picker-time-header .group-item:nth-child(1) input'),
      timeInput: pickerss[0].querySelector('.picker-time-header .group-item:nth-child(2) input'),
    }
    const right = {
      dateInput: pickerss[1].querySelector('.picker-time-header .group-item:nth-child(1) input'),
      timeInput: pickerss[1].querySelector('.picker-time-header .group-item:nth-child(2) input'),
    }
    await nextTick()
    // both input shows correct value
  ...
  it('input date', async () => {
    ...
    await nextTick()
    const pickerss = document.querySelectorAll('.picker-body-header-wrapper .picker-time-header')
    const leftDateInput = pickerss[0].querySelector('.picker-time-header .group-item:nth-child(1) input') as HTMLInputElement
    const rightDateInput = pickerss[0].querySelector('.picker-time-header .group-item:nth-child(1) input') as HTMLInputElement
    ...
    triggerEvent(rightCell, 'click', true)
    await nextTick()
    const btn = document.querySelector(`.picker-footer .${prefix}-button.primary`) as HTMLElement
    btn.click()
    await nextTick()
    const vm = wrapper.vm as any
    expect(vm.value.map(_ => dayjs(_).format(formatStr)))
  ...
  it('select time', async () => {
    ...
    input.trigger('focus')
    await nextTick()
    const timeInput = document.querySelectorAll('.picker-time-header input')[1] as HTMLInputElement
    timeInput.blur()
    timeInput.focus()
    timeInput.blur()
    await nextTick()
  ...
  it('confirm honors disabledDate', async () => {
    ...
    btn.click()
    await nextTick()
    const rangePanel = document.querySelector('.range-picker')!
  ...
  it('selectableRange', async () => {
    ...
    await nextTick()
    const pickerss = document.querySelectorAll('.picker-body-header-wrapper .picker-time-header')
    const leftDateInput = pickerss[0].querySelector('.picker-time-header .group-item:nth-child(2) input') as HTMLInputElement
    const rightDateInput = pickerss[1].querySelector('.picker-time-header .group-item:nth-child(2) input') as HTMLInputElement
    leftDateInput.blur()
    leftDateInput.focus()
    await nextTick()
    const listleft = document.querySelectorAll('.time-spinner .spinner-list')
    ...
    await nextTick()
    const listright = document.querySelectorAll('.picker-time-header.is-right .spinner-list')
  ...
  it('select same date, different time', async () => {
    ...
    triggerEvent(leftCell, 'click', true)
    await nextTick()
    const leftTimeInput = document.querySelectorAll('.picker-body-header-wrapper input')[1] as HTMLInputElement
    leftTimeInput.blur()
    leftTimeInput.focus()
    await nextTick()
    const leftList = document.querySelectorAll('.time-spinner .spinner-list')
    ...
    await nextTick();
    (document.querySelector(`.${prefix}-time-picker .confirm`) as HTMLElement).click()
    await nextTick()
    const rightTimeInput = document.querySelectorAll('.picker-body-header-wrapper input')[3] as HTMLInputElement
    rightTimeInput.blur()
    rightTimeInput.focus()
    await nextTick()
    const rightList = document.querySelectorAll('.is-right .time-spinner .spinner-list')
    // auto set left time to right time
    expect(rightList[0].querySelector('.spinner-item.active')?.innerHTML).toBe(leftSelect[0])
    expect(rightList[1].querySelector('.spinner-item.active')?.innerHTML).toBe(leftSelect[1])
    expect(rightList[2].querySelector('.spinner-item.active')?.innerHTML).toBe(leftSelect[2])
    ...
    await nextTick();
    (document.querySelector(`.is-right .${prefix}-button.confirm`) as HTMLElement).click()
    await nextTick();
    (document.querySelector(`.picker-footer .${prefix}-button.primary`) as HTMLElement).click()
    await nextTick()
    const vm = wrapper.vm as any
...
describe('form item accessibility integration', () => {
  it('automatic id attachment', async () => {
    ...
    const datePickerInput = wrapper.find('.input-inner')
  ...
  it('specified id attachment', async () => {
    const wrapper = _mount(
      `<hl-form-item label="Foobar" data-test-ref="item">
        <hl-date-picker type="datetime" id="foobar" />
      </hl-form-item>`,
      () => ({}),
    )
