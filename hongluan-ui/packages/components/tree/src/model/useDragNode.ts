...
export function useDragNodeHandler({ props, ctx, el$, dropIndicator$, store }) {
  ...
  const treeNodeDragOver = ({ event, treeNode }: DragOptions) => {
    ...
    const iconPosition = dropNode.$el.querySelector('.tree-node-arrow').getBoundingClientRect()
  ...
  const treeNodeDragEnd = (event: DragEvent) => {
    ...
    if (draggingNode && dropNode) {
      ...
      if (dropType !== 'none') {
        draggingNodeChecked && store.value.setChecked(draggingNode.node.data, false)
      ...
      if (dropType !== 'none') {
        ...
        draggingNodeChecked && store.value.setChecked(draggingNodeCopy.data, true)
      }
