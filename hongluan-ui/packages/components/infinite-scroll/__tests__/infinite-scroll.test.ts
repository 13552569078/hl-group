...
import tick from '@hongluan-ui/test-utils/tick'
...
let clientHeightRestore: any = null
let scrollHeightRestore: any = null
...
beforeAll(() => {
  ...
  scrollHeightRestore = defineGetter(window.HTMLElement.prototype, 'scrollHeight', function (this: HTMLElement) {
...
describe('InfiniteScroll', () => {
  ...
  test('callback will not be triggered infinitely', async () => {
    ...
    wrapper.vm.$refs.ulRef.InfiniteScroll.instance.count++
    await nextTick()
    expect(countListItem(wrapper)).toBe(INITIAL_VALUE)
  })
