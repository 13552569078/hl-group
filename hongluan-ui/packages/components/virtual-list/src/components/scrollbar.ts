import { computed, defineComponent, ref, reactive, onBeforeUnmount, watch, h, withModifiers, unref } from 'vue'
import { BAR_MAP } from '@hongluan-ui/components/scrollbar'
import { rAF, cAF } from '@hongluan-ui/utils'
...
const ScrollBar = defineComponent({
  ...
  setup(props, { emit }) {
    const { namespace } = useNamespace('virtual-scrollbar')
    ...
    const onThumbMouseDown = (e: Event) => {
      ...
      if ((e as KeyboardEvent).ctrlKey || [1, 2].includes((e as MouseEvent).button)) {
        return
      }
