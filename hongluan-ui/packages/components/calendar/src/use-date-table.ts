...
import { rangeArr } from '@hongluan-ui/components/time-picker'
import { WEEK_DAYS } from '@hongluan-ui/constants'
...
export const useDateTable = (
  ...
  const weekDays = computed(() => {
    ...
    if (start === 0) {
      return WEEK_DAYS.map(_ => t(`hl.datepicker.weeks.${_}`))
    } else {
      return WEEK_DAYS.slice(start)
        .concat(WEEK_DAYS.slice(0, start))
        .map(_ => t(`hl.datepicker.weeks.${_}`))
    }
  })
