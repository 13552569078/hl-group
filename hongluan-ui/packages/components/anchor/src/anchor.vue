...
import { isNumber, throttleByRaf } from '@hongluan-ui/utils'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const getContainer = (targetContainer?: HTMLElement) => {
      let scrollContainer: string | HTMLElement | Window = targetContainer
      if (!scrollContainer) {
    ...
    const getContainerElement = (): HTMLElement => {
      ...
      if (props.scrollContainer && props.scrollContainer !== window) {
        return props.scrollContainer as HTMLElement
      }
    ...
    const scrollIntoView = (hash: string) => {
      ...
      try {
        ...
        if (!node) return
        let block = props.boundary as BoundaryPosition
        let diff = 0
        if (isNumber(props.boundary)) {
          block = 'start'
          diff = props.boundary as number
        }
    ...
    const getFirstInViewportEle = (): HTMLElement | undefined => {
      const boundary = isNumber(props.boundary) ? props.boundary as number : 0
