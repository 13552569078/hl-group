...
import { HlFormItem as FormItem } from '@hongluan-ui/components/form-item'
...
describe('Autocomplete.vue', () => {
  ...
  test('popperClass', async () => {
    ...
    expect(
      document.body.querySelector(`.${prefix}-popper`)?.classList.contains('error'),
    ).toBe(true)
    ...
    expect(
      document.body.querySelector(`.${prefix}-popper`)?.classList.contains('success'),
    ).toBe(true)
  })

  test('teleported', async () => {
    _mount({ teleported: false })
    expect(document.body.querySelector('.popper-mask')).toBeNull()
  })
  ...
  test('hideLoading', async () => {
    ...
    expect(document.body.querySelector(`.${prefix}-icon-loading`)).toBeDefined()
    await wrapper.setProps({ hideLoading: true })
    expect(document.body.querySelector(`.${prefix}-icon-loading`)).toBeNull()
  })
  ...
  test('fitInputWidth', async () => {
    ...
    const inputDom = wrapper.find(`.${prefix}-input`).element
