import { hasOwn } from '@hongluan-ui/utils'
...
type INT_HEX_MAP_KEY = keyof typeof INT_HEX_MAP

const hexOne = function (value: number) {
  value = Math.min(Math.round(value), 255)
  const high = Math.floor(value / 16) as INT_HEX_MAP_KEY
  const low = value % 16 as INT_HEX_MAP_KEY
...
type HEX_INT_MAP_KEY = keyof typeof HEX_INT_MAP

const parseHexChannel = function (hex: string) {
  if (hex.length === 2) {
    return (
      (HEX_INT_MAP[hex[0].toUpperCase() as HEX_INT_MAP_KEY] || +hex[0]) * 16 +
      (HEX_INT_MAP[hex[1].toUpperCase() as HEX_INT_MAP_KEY] || +hex[1])
    )
  }

  return HEX_INT_MAP[hex[1].toUpperCase() as HEX_INT_MAP_KEY] || +hex[1]
}
...
const rgb2hsv = function (r: string | number, g: string | number, b: string | number) {
  r = bound01(r, 255)
  g = bound01(g, 255)
  b = bound01(b, 255)
...
const hsv2rgb = function (h: string | number, s: string | number, v: string | number) {
  h = bound01(h, 360) * 6
  s = bound01(s, 100)
  v = bound01(v, 100)
