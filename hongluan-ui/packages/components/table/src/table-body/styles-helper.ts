...
function useStyles<T>(props: Partial<TableBodyProps<T>>) {
  ...
  const getRowClass = (row: T, rowIndex: number) => {
    ...
    if (props.stripe) {
      if (((typeof props.stripe === 'boolean' && props.stripe) || props.stripe === 'odd') && rowIndex % 2 === 1) {
        classes.push('table-row-striped')
      } else if (props.stripe === 'even' && rowIndex % 2 === 0) {
        classes.push('table-row-striped')
      }
    }
    ...
    if (props.store.states.selection.value.indexOf(row) > -1) {
