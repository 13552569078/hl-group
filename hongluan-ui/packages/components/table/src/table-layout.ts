...
import { getScrollBarWidth } from '@hongluan-ui/utils'
...
class TableLayout<T> {
  ...
  updateScrollY() {
    ...
    if (this.table.vnode.el && bodyWrapper) {
      ...
      if (this.bodyHeight.value === null) {
        ...
        const body = bodyWrapper.querySelector('.table-body') as HTMLElement
  ...
  updateElsHeight() {
    ...
    const headerTrElm: HTMLElement = headerWrapper
      ? headerWrapper.querySelector('.table-header tr')
      : null
    ...
    if (flexColumns.length > 0 && fit) {
      ...
      if (bodyMinWidth <= bodyWidth - scrollYWidth) {
        ...
        if (flexColumns.length === 1) {
          ...
          flexColumns[0].realWidth =
            Number(flexColumns[0].minWidth || 80) + totalFlexWidth - noneFirstWidth
        }
      } else {
