import { onMounted, computed, ref, watchEffect, watch, unref, nextTick } from 'vue'
...
function useStyle<T>(
  ...
  const bindEvents = () => {
    ...
    if (table.refs.scrollBarRef.wrapRef) {
      useEventListener(table.refs.scrollBarRef.wrapRef, 'scroll', syncPostion, {
        passive: true,
      })
    }
    if (props.fit) {
  ...
  const tableSize = computed(() => {
    return props.size || $HL.value.size
  })
