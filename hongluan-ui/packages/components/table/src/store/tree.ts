...
function useTree<T>(watcherData: WatcherPropsData<T>) {
  ...
  const updateTreeData: (ifChangeExpandRowKeys?: boolean, ifExpandAll?: boolean) => void =
    (ifChangeExpandRowKeys = false, ifExpandAll = instance.store?.states.defaultExpandAll.value) => {
  ...
  watch(() => expandRowKeys.value, () => { updateTreeData(true) })
