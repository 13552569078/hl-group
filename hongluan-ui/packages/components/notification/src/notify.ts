...
import { isVNode, isElement, isString, debugWarn } from '@hongluan-ui/utils'
...
const notify: NotifyFn & Partial<Notify> & { _context: AppContext | null; } =
  function (options = {}, context: AppContext | null = null) {
    ...
    const instance: any = vm.component
    watch(() => instance.proxy.icon, (newVal, oldVal) => {
      if (isVNode(newVal)) {
      // Override slots since icon is vnode type.
