...
const MENU = '.cascader-menu'
const NODE = '.cascader-node'
const VALID_NODE = '.cascader-node:not(.is-disabled)'
const EXPAND_ARROW = '.arrow-right.cascader-node-postfix'
const CHECKBOX = `.${prefix}-checkbox`
const RADIO = `.${prefix}-radio`
...
describe('CascaderPanel.vue', () => {
  ...
  test('with default value', async () => {
    ...
    expect(hzNode.find('.checked').exists()).toBe(true)
  ...
  test('emit value only, falsely value[0, false]', async () => {
    const wrapper = _mount({
      template: `
        <cascader-panel
          v-model="value"
  ...
  test('lazy load', async () => {
    ...
    expect(firstOption.find(`.${prefix}-spinner`).exists()).toBe(true)
    vi.runAllTimers()
    await nextTick()
    expect(firstOption.find(`.${prefix}-spinner`).exists()).toBe(false)
