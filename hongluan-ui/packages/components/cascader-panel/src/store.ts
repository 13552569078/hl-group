...
export default class Store {
  ...
  getNodeByValue (value: CascaderNodeValue | CascaderNodePathValue, leafOnly = false): Nullable<Node> {
    ...
    const node = this.getFlattedNodes(leafOnly)
      .find(node => isEqual(node.value, value) || isEqual(node.pathValues, value))
  ...
  getSameNode (node: Node): Nullable<Node> {
    ...
    const node_ = this.getFlattedNodes(false)
      .find(({ value, level }) => isEqual(node.value, value) && node.level === level)
