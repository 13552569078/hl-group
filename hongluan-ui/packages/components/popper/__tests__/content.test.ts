...
import HlContent from '../src/content.vue'
...
describe('<PopperContent />', () => {
  describe('with triggerRef provided', () => {
    const triggerKls = 'popper-trigger'
    let wrapper: any
    beforeEach(() => {
    ...
    it('should mount the component correctly and set popperInstance correctly', async () => {
      ...
      expect(wrapper.classes()).toEqual([`${prefix}-popper`])
    ...
    it('should be able to be pure and themed', async () => {
      ...
      expect(wrapper.classes()).toEqual([`${prefix}-popper`, 'is-pure', 'custom'])
    })
