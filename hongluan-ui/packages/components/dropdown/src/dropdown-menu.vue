...
import { composeRefs, composeEventHandlers } from '@hongluan-ui/utils'
...
export default defineComponent({
  ...
  setup(props) {
    const { namespace } = useNamespace('dropdown-menu')
    const { _hlDropdownSize } = useDropdown()
    const size = _hlDropdownSize.value
    ...
    const dropdownKls = computed(() => {
      return [namespace.value, size?.value]
    })
