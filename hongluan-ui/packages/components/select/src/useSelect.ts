import { nextTick, computed, watch, ref, reactive, triggerRef, shallowRef, toRaw } from 'vue'
...
import { useLocale, useConsistentProp, useFormItem } from '@hongluan-ui/hooks'
import { isClient } from '@vueuse/core'
import { scrollIntoView, isKorean, debugWarn, isFunction, isNumber, isString } from '@hongluan-ui/utils'
import { FormItemEvents } from '@hongluan-ui/tokens'

import type { ComponentPublicInstance, SetupContext } from 'vue'
import type HlTooltip from '@hongluan-ui/components/tooltip'
import type { SelectOptionProxy, QueryChangeCtx } from './token'
...
export function useSelectStates(props: any) {
  ...
  return reactive({
    ...
    inputHovering: false,
    cachedPlaceHolder: '',
    currentPlaceholder: t('hl.select.placeholder'),
...
export const useSelect = (props: any, states: States, ctx: SetupContext<any>) => {
  const { t } = useLocale()
  ...
  const tooltipRef = ref<InstanceType<typeof HlTooltip> | null>(null)
  ...
  const readonly = computed(() => !props.filterable || !states.visible)
  ...
  const iconClass = computed(() => dropMenuVisible.value && props.suffixTransition ? 'select-caret is-open' : 'select-caret')
  const showIcon = computed(() => props.remote && props.filterable && !props.remoteShowSuffix ? false : true)

  const debounce = computed(() => props.remote ? 300 : 0)

  const emptyText = computed(() => {
    if (props.loading) {
      return props.loadingText || t('hl.select.loading')
    } else {
      if (props.remote && states.query === '' && states.options.size === 0) return false
      if (props.filterable && states.query && states.options.size > 0 && states.filteredOptionsCount === 0) {
        return props.noMatchText || t('hl.select.noMatch')
      }
      if (states.options.size === 0) {
        return props.noDataText || t('hl.select.noData')
      }
    }
    return null
  })
  ...
  watch(
    ...
    () => {
      ...
      if ((<HTMLInputElement[]>[]).indexOf.call(inputs, document.activeElement as HTMLInputElement) === -1) {
        setSelected()
      }
  ...
  const setSelected = () => {
    if (!props.multiple) {
      const option = getOption(props.modelValue) as any
    ...
    setTimeout(() => {
      ...
      if (!props.multiple) {
        ...
        if (states.selected.length > 0) {
          states.hoverIndex = Math.min.apply(null, states.selected.map((selected: any) => {
  ...
  const deletePrevTag = (e: Event) => {
    if ((e.target as HTMLInputElement)?.value.length <= 0 && !toggleLastOptionHitState()) {
  ...
  const handleOptionSelect = (option: any, byClick: boolean | undefined) => {
    if (props.multiple) {
      const value = (props.modelValue || []).slice()
  ...
  const scrollToOption = (option: any) => {
    ...
    if (tooltipRef.value && target) {
      const menu = tooltipRef.value?.popperRef?.contentRef?.querySelector?.('.select-dropdown-wrap')
      if (menu) {
