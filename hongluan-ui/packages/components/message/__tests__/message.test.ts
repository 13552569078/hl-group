...
type MessageInstance = ComponentPublicInstance<{ visible: boolean; iconName: string; customStyle: CSSProperties; }>
...
describe('Message.vue', () => {
  ...
  describe('Message.type', () => {
    test('should be able to render typed messages', () => {
      for (const type of ['success', 'warning', 'info', 'error', 'danger'] as const) {
        const wrapper = _mount({ props: { type } })
        const exists = wrapper.find(`.${prefix}-icon svg`).exists()
    ...
    test('should not be able to render invalid type icon', () => {
      ...
      expect(wrapper.find(`.${prefix}-icon svg[aria-labelledby=${type}]`).exists()).toBe(false)
    })
  })
  ...
  describe('event handlers', () => {
    test('it should be able to close the message by clicking close button', async () => {
      ...
      const closeBtn = wrapper.find('.message-close')
