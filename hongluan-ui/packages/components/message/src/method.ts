...
import { messageConfig } from '@hongluan-ui/components/config-provider/src/config-provider'
...
const createMessage = (
  ...
  watch(() => (vm.proxy as MessageInstance).icon, (newVal, oldVal) => {
    if (isVNode(newVal)) {
      // Override slots since message is vnode type.
      vnode.component!.slots.icon = () => [newVal]
    } else if(isVNode(oldVal) && !isVNode(newVal)){
      delete vnode.component!.slots.icon
    }
  }, {
    immediate: true,
  })
