...
import { supportTouch } from '@hongluan-ui/utils'
import { useGlobalConfig, useNamespace } from '@hongluan-ui/hooks'

import type { ObjectDirective } from 'vue'
import type { DirectiveBinding } from '@vue/runtime-core'

const { namespace } = useNamespace('ripple')
...
function computeRippleStyles(element: RippleHTMLElement, event: TouchEvent | MouseEvent): RippleStyles {
  ...
  const localX: number = (event as any).touches ? (event as TouchEvent).touches[0].clientX : (event as MouseEvent).clientX - left
  const localY: number = (event as any).touches ? (event as TouchEvent).touches[0].clientY : (event as MouseEvent).clientY - top
...
function createRipple(this: RippleHTMLElement, event: TouchEvent | MouseEvent) {
  ...
  const task = () => {
    ...
    ripple.classList.add(namespace.value)
...
function removeRipple(this: RippleHTMLElement) {
  ...
  const task = () => {
    const ripples: NodeListOf<RippleHTMLElement> = this.querySelectorAll(`.${namespace.value}`)
    if (!ripples.length) {
      return
    }
